  ...
  <div class="page-container">
    ...
    >
      ...
      <el-table-column label="预约" width="140">
        <template #default="{ row }">
          <el-progress
            :percentage="row.capacity ? Math.round((row.booked_count / row.capacity) * 100) : 0"
            :color="row.booked_count >= row.capacity ? '#f56c6c' : '#667eea'"
          />
          <span style="font-size: 12px; color: #909399"
            >{{ row.booked_count }}/{{ row.capacity }}</span
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      <!-- 单条排期表单 -->
      <el-form
        v-if="isEdit || scheduleMode === 'single'"
        ref="formRef"
        :model="form"
        label-width="100px"
        :rules="rules"
        ...
        <el-form-item label="日期" prop="start_date">
          <div style="display: flex; align-items: center; gap: 8px; width: 100%">
            <el-date-picker
              ...
              placeholder="选择日期"
              style="flex: 1"
              :disabled-date="disabledSingleDate"
              @change="updateSingleEndTime"
            />
            <el-tooltip
              v-if="!isEdit"
              content="勾选后可选择过去日期，用于补录历史课程数据"
              placement="top"
            >
              <el-checkbox v-model="allowPastDate" style="white-space: nowrap">
                补录历史排期
              </el-checkbox>
            </el-tooltip>
          </div>
        ...
        <el-form-item label="日期范围" prop="dateRange">
          <div style="display: flex; align-items: center; gap: 8px; width: 100%">
            <el-date-picker
              ...
              style="flex: 1"
              :disabled-date="disabledBatchDate"
            />
      ...
      <template #footer>
        ...
        <template v-else>
          <el-button v-if="batchPreview.length === 0" type="info" @click="generatePreview">
            生成预览
          </el-button>
          <el-button v-else type="primary" :loading="batchSubmitting" @click="handleBatchSubmit">
            确认创建
          </el-button>
        </template>
      </template>
    ...
    <el-dialog v-model="studentDialogVisible" title="学员列表" width="500px" destroy-on-close>
      ...
      >
        <el-table-column label="学员" width="180">
          <template #default="{ row }">
            {{ row.student_nickname || `学员#${row.student_id}` }}
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
    ...
    <el-dialog v-model="cancelVisible" title="取消排期" width="500px" destroy-on-close>
      ...
      >
        <template #title> 该排期已有 {{ cancelTarget.booked_count }} 名学员预约 </template>
        <div>
...
function getCourseName(id: number) {
  return courseMap.value[id] || `课程#${id}`
}
function getCourseDuration(id: number) {
  return courseDurationMap.value[id] || 0
}
function getTeacherName(id: number) {
  return teacherMap.value[id] || `教师#${id}`
}
function getClassroomName(id: number | null) {
  return id ? classroomMap.value[id] || `教室#${id}` : '-'
}
...
function updateSingleEndTime() {
  ...
  // 处理日期可能是字符串的情况
  const startDate =
    typeof form.value.start_date === 'string'
...
async function handleSingleSubmit() {
  ...
  if (
    ...
    if (courseDuration !== scheduleDuration) {
      ElMessage.error(
        `排期时长(${scheduleDuration}分钟)与课程时长(${courseDuration}分钟)不匹配，请调整结束时间`
      )
      return
    }
  }
...
async function confirmCancel() {
  ...
  const reason =
    cancelForm.value.reason === '其他' ? cancelForm.value.detail : cancelForm.value.reason
  ...
  try {
    ...
    if (result.total_bookings > 0) {
      ElMessage.success(
        `排期已取消，已处理 ${result.success_count}/${result.total_bookings} 个学员预约，课时已退还`
      )
    } else {
...
async function handleBatchDelete() {
  ...
  if (hasBookedNormalRows.length > 0) {
    ElMessage.warning(
      `选中的 ${selectedCount} 个排期中，有 ${hasBookedNormalRows.length} 个待上课排期已有学员预约，请先取消排期后再删除`
    )
    return
  }
...
async function generatePreview() {
  ...
  const { course_id, dateRange, weekdays, startTime, teacher_id, classroom_id, capacity } =
    batchForm.value
