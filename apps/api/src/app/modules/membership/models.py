...
class MembershipCardProduct(Base, TenantMixin, TimestampMixin):
    ...
    price: Mapped[Decimal] = mapped_column(
        Numeric(12, 2), nullable=False, default=0, comment="售价"
    )
...
class MembershipCard(Base, TenantMixin, TimestampMixin):
    ...
    product_id: Mapped[int | None] = mapped_column(
        BigInteger,
        ForeignKey("membership_card_products.id", ondelete="SET NULL"),
        nullable=True,
        comment="关联产品ID",
    )
    ...
    used_credits: Mapped[int] = mapped_column(
        Integer, nullable=False, default=0, comment="已使用次数"
    )
    ...
    frozen_until: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), comment="冻结到期时间（NULL表示无限期冻结）"
    )
    ...
    cancelled_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), comment="作废时间"
    )
...
class MembershipCardTransaction(Base, TenantMixin, TimestampMixin):
    ...
    card_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("membership_cards.id", ondelete="RESTRICT"),
        nullable=False,
        comment="会员卡ID",
    )
    ...
    change_amount: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="变动数量（正数增加，负数扣减）"
    )
...
class MembershipCardFreeze(Base, TenantMixin, TimestampMixin):
    ...
    unfrozen_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), comment="解冻时间"
    )
    operator_id: Mapped[int | None] = mapped_column(BigInteger, comment="操作人ID")
