...
class Classroom(Base, TenantMixin, TimestampMixin):
    ...
    status: Mapped[int] = mapped_column(
        SmallInteger,
        nullable=False,
        default=ClassroomStatus.ACTIVE.value,
    )

    __table_args__ = (Index("uq_classrooms_tenant_name", "tenant_id", "name", unique=True),)
...
class Course(Base, TenantMixin, TimestampMixin):
    ...
    status: Mapped[int] = mapped_column(
        SmallInteger,
        nullable=False,
        default=CourseStatus.ONLINE.value,
    )
    deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    __table_args__ = (
        Index(
            "idx_courses_tenant_status",
            "tenant_id",
            "status",
