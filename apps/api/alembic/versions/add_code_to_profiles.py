...
def upgrade() -> None:
    op.add_column(
        "teacher_profiles",
        sa.Column("teacher_code", sa.String(20), nullable=True, comment="教师编号"),
    )
    op.create_index("idx_teacher_profiles_code", "teacher_profiles", ["teacher_code"], unique=True)

    op.add_column(
        "student_profiles",
        sa.Column("student_code", sa.String(20), nullable=True, comment="学员编号"),
    )
    ...
    for row in teacher_rows:
        conn.execute(
            sa.text("UPDATE teacher_profiles SET teacher_code = :code WHERE id = :id"),
            {"id": row[0], "code": generate_code("T")},
        )

    student_rows = conn.execute(sa.text("SELECT id FROM student_profiles")).fetchall()
    for row in student_rows:
        conn.execute(
            sa.text("UPDATE student_profiles SET student_code = :code WHERE id = :id"),
            {"id": row[0], "code": generate_code("S")},
        )
